    ...
    try {
      ...
      if (entryResult && entryResult.success) {
        ...
        if (chat_id) {
          ...
          try { try { await sendTelegramMessage(errorMessage, accountId); } catch (e) { console.warn("[TELEGRAM] Erro:", e.message); } } catch (telegramError) { console.warn("[TELEGRAM] Erro ao enviar mensagem:", telegramError.message); }
        }
