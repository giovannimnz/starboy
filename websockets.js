...
async function startUserDataStream(db, accountId = 1) {
  try {
    ...
    if (listenKey) {
      ...
      ws.on('open', () => {
        console.log(`[WEBSOCKET] Conexão de dados do usuário estabelecida com sucesso para conta ${accountId}`);
      ...
      ws.on('message', async (data) => {
        try {
          ...
          console.error(`[WEBSOCKET] Erro ao processar mensagem para conta ${accountId}:`, error);
        }
      });
      
      ws.on('error', (error) => {
        console.error(`[WEBSOCKET] Erro na conexão de dados do usuário para conta ${accountId}:`, error);
        restartUserDataStream(db, accountId);
      });
