...
async function dashboardRoutes(fastify, options) {
  ...
  fastify.get('/dashboard/account/:id/saldo', {
    ...
    const db = await getDatabaseInstance();
    try {
      const result = await db.query(`SELECT saldo, saldo_base_calculo, ultima_atualizacao FROM contas WHERE id = $1`, [id]);
      if (result.rows.length === 0) {
        return reply.status(404).send({ error: 'Conta não encontrada' });
      }
      reply.send({ success: true, saldo: result.rows[0].saldo, saldo_base_calculo: result.rows[0].saldo_base_calculo, ultima_atualizacao: result.rows[0].ultima_atualizacao });
    } catch (error) {
  ...
  fastify.post('/dashboard/account/:id/selecionar', {
    ...
    const db = await getDatabaseInstance();
    try {
      const result = await db.query(`SELECT id, nome FROM contas WHERE id = $1`, [id]);
      if (result.rows.length === 0) {
        return reply.status(404).send({ error: 'Conta não encontrada' });
      }
      // Aqui você pode salvar a seleção em uma tabela, sessão, cache, etc.
      // Exemplo: reply.send apenas confirma a seleção
      reply.send({ success: true, message: `Conta ${result.rows[0].nome} selecionada para o dashboard.`, conta: result.rows[0] });
    } catch (error) {
  ...
  fastify.get('/dashboard/account/:id/saldo-futuros', async (request, reply) => {
    ...
    const db = await getDatabaseInstance();
    try {
      const result = await db.query(`SELECT saldo_futuros, saldo_base_calculo_futuros, ultima_atualizacao FROM contas WHERE id = $1`, [id]
      );
      if (result.rows.length === 0) {
        return reply.status(404).send({ error: 'Conta não encontrada' });
      }
      reply.send({
        success: true,
        saldo_futuros: result.rows[0].saldo_futuros,
        saldo_base_calculo_futuros: result.rows[0].saldo_base_calculo_futuros,
        ultima_atualizacao: result.rows[0].ultima_atualizacao
      });
    } catch (error) {
  ...
  fastify.get('/dashboard/account/:id/saldo-spot', async (request, reply) => {
    ...
    const db = await getDatabaseInstance();
    try {
      const result = await db.query(`SELECT saldo_spot, saldo_base_calculo_spot, ultima_atualizacao FROM contas WHERE id = $1`, [id]
      );
      if (result.rows.length === 0) {
        return reply.status(404).send({ error: 'Conta não encontrada' });
      }
      reply.send({
        success: true,
        saldo_spot: result.rows[0].saldo_spot,
        saldo_base_calculo_spot: result.rows[0].saldo_base_calculo_spot,
        ultima_atualizacao: result.rows[0].ultima_atualizacao
      });
    } catch (error) {
