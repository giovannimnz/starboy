...
try:
    from ..boolean_validator import validate_signals_analysis_data, normalize_boolean
except ImportError as e:
    print(f"[AVISO] Não foi possível importar validador de booleanos: {e}")
    validate_signals_analysis_data = lambda x: x
...
class DIVAPAnalyzer:
    ...
    def save_analysis_result(self, result: Dict) -> None:
        if "error" in result:
            ...
            try:
                update_query = """
                    ...
                    WHERE id = %s
                """
                # Usar valores booleanos Python explícitos
                self.cursor.execute(update_query, (False, True, f"Erro na análise: {error_msg}", signal_id))
                self.conn.commit()
        ...
        try:
            # Validar campos booleanos no resultado antes de salvar
            validated_result = validate_signals_analysis_data(result)
            
            rsi_to_save = validated_result.get("rsi")
            volume_sma_to_save = validated_result.get("volume_sma")
            values = (
                validated_result.get("signal_id"), validated_result.get("is_bull_divap", False),
                validated_result.get("is_bear_divap", False), validated_result.get("divap_confirmed", False),
                0 if pd.isna(rsi_to_save) else rsi_to_save,
                validated_result.get("volume", 0),
                0 if pd.isna(volume_sma_to_save) else volume_sma_to_save,
                validated_result.get("high_volume", False), validated_result.get("bull_div", False),
                validated_result.get("bear_div", False), validated_result.get("message", ""),
                validated_result.get("bull_reversal_pattern", False),
                validated_result.get("bear_reversal_pattern", False), datetime.now()
            )
            ...
            sql = """
                ...
                ON CONFLICT (signal_id) DO UPDATE SET
                    is_bull_divap=EXCLUDED.is_bull_divap, is_bear_divap=EXCLUDED.is_bear_divap,
                    divap_confirmed=EXCLUDED.divap_confirmed, rsi=EXCLUDED.rsi, volume=EXCLUDED.volume, 
                    volume_sma=EXCLUDED.volume_sma, high_volume=EXCLUDED.high_volume, 
                    bull_div=EXCLUDED.bull_div, bear_div=EXCLUDED.bear_div, message=EXCLUDED.message,
                    bull_reversal_pattern=EXCLUDED.bull_reversal_pattern,
                    bear_reversal_pattern=EXCLUDED.bear_reversal_pattern, analysis_type=EXCLUDED.analysis_type,
                    analyzed_at=EXCLUDED.analyzed_at
            """
            
            # Adicionar analysis_type='trade' para divap_check.py
            values_with_type = values + ('trade',)
            self.cursor.execute(sql, values_with_type)
            self.conn.commit()
            logger.info(f"Análise do sinal {result.get('signal_id')} salva na tabela signals_analysis")
        except Exception as e:
        ...
        try:
            ...
            # Garantir que os valores booleanos sejam do tipo correto
            is_divap_confirmed = normalize_boolean(is_divap_confirmed)
