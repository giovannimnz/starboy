...
def update_exchange_info_database(exchange_name):
    ...
    try:
        ...
        for symbol, symbol_data in api_symbols_map.items():
            ...
            # ✅ VERIFICAR SE symbol_id É VÁLIDO ANTES DE CONTINUAR
            if symbol_id is None or symbol_id <= 0:
                print(f"[{datetime.datetime.now().strftime('%d-%m-%Y | %H:%M:%S')}] [EXCHANGE-INFO] ❌ Erro: symbol_id inválido ({symbol_id}) para símbolo {symbol}")
                continue
