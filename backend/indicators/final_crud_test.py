...
def test_signals_analysis_crud():
    ...
    try:
        ...
        INSERT INTO signals_analysis (
            ...
            analysis_type, rsi, volume, high_volume, symbol, side
        ) VALUES (
            99999, true, false, true,
            true, false,
            'trade', 65.5, 1000000.0, true, 'BTCUSDT', 'BUY'
        ) RETURNING id
        """
        ...
        if record:
            print(f"   ✅ SELECT OK - analysis_type: {record['analysis_type']}, symbol: {record['symbol']}")
        else:
            print("   ❌ SELECT FALHOU")
        ...
        cursor.execute("""
            ...
            SET analysis_type = 'backtest', rsi = 75.0, side = 'SELL' 
            WHERE id = %s
        """, (record_id,))
        conn.commit()
        
        cursor.execute("SELECT analysis_type, rsi, side FROM signals_analysis WHERE id = %s", (record_id,))
...
def test_backtest_signals_crud():
    ...
    try:
        ...
        cursor.execute("""
            ...
            SET entry_price = 51000.0, status = 'CLOSED'
            WHERE id = %s
        """, (record_id,))
        conn.commit()
        
        cursor.execute("SELECT entry_price, status FROM backtest_signals WHERE id = %s", (record_id,))
        updated_record = cursor.fetchone()
...
def test_backtest_results_crud():
    ...
    try:
        ...
        cursor.execute("""
            ...
            SET total_pnl = 1750.25, total_profit = 1765.25 
            WHERE id = %s
        """, (record_id,))
        conn.commit()
        
        cursor.execute("SELECT total_pnl, total_profit FROM backtest_results WHERE id = %s", (record_id,))
        updated_record = cursor.fetchone()
...
def test_analysis_type_functionality():
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO signals_analysis (
                signal_id, is_bull_divap, analysis_type, symbol, side
            ) VALUES (
                99001, true, 'trade', 'BTCUSDT', 'BUY'
            ) RETURNING id
        """)
        ...
        cursor.execute("""
            INSERT INTO signals_analysis (
                signal_id, is_bear_divap, analysis_type, symbol, side
            ) VALUES (
                99002, true, 'backtest', 'ETHUSDT', 'SELL'
            ) RETURNING id
        """)
        test_records.append(cursor.fetchone()['id'])
