...
// ✅ SISTEMA DE LOGGING ESPECÍFICO PARA REVERSE.JS
// Garante que todos os logs sejam sempre exibidos, exceto logs de WebSocket de preços
const reverseLog = (...args) => {
  // Sempre exibe logs do reverse.js, mesmo se outros logs estiverem desabilitados
  console.log(...args);
};

const reverseError = (...args) => {
  // Sempre exibe erros do reverse.js
  console.error(...args);
};

const reverseWarn = (...args) => {
  // Sempre exibe warnings do reverse.js
  console.warn(...args);
};

// ✅ FUNÇÃO PARA LOGS DE WEBSOCKET DE PREÇOS (SUPRIMIDOS)
const priceWSLog = (...args) => {
  // Suprime logs de WebSocket de preços para evitar spam
  // Descomente a linha abaixo se quiser ver os logs de preços
  // console.log(...args);
};
...
function createPositionBuffer(signal, totalSize, averagePrice, accountId) {
  ...
  reverseLog(`[BUFFER] Posição adicionada ao buffer: ${signal.symbol} (${totalSize.toFixed(8)})`);
  return positionData;
}
...
function addOrderToBuffer(orderData, signalId) {
  ...
  reverseLog(`[BUFFER] Ordem adicionada ao buffer: ${orderData.orderId} (${orderData.type})`);
...
function updateOrderInBuffer(orderData) {
  ...
  if (bufferedOrder) {
    ...
    reverseLog(`[BUFFER] Ordem atualizada no buffer: ${orderId} (${bufferedOrder.status})`);
    return bufferedOrder;
  }
...
async function insertBufferedDataToDB(signalId, accountId, connection) {
  try {
    reverseLog(`[BUFFER] 📤 Iniciando inserção no banco para signal ${signalId}...`);
    ...
    if (positionData) {
      reverseLog(`[BUFFER] 📊 Inserindo posição: ${positionData.simbolo}`);
      ...
      reverseLog(`[BUFFER] ✅ Posição inserida com ID: ${positionId}`);
    ...
    reverseLog(`[BUFFER] 📋 Inserindo ${signalOrders.length} ordens...`);
    ...
    for (const [orderId, orderData] of signalOrders) {
      try {
        ...
        reverseLog(`[BUFFER] ✅ Ordem inserida: ${orderId} (${orderData.tipo_ordem_bot})`);
        
        // Remover do buffer após inserção
        orderBuffer.delete(orderId);
      } catch (orderError) {
        reverseError(`[BUFFER] ❌ Erro ao inserir ordem ${orderId}:`, orderError.message);
      }
    }
    
    reverseLog(`[BUFFER] 🎉 Inserção completa para signal ${signalId}`);
    return { success: true, positionId };
    
  } catch (error) {
    reverseError(`[BUFFER] ❌ Erro na inserção do buffer:`, error.message);
    throw error;
  }
}
...
async function executeReverse(signal, currentPrice, accountId) {
  ...
  const orderUpdateHandler = (orderMsg) => {
    ...
    if (executionTime < executionStartTime) {
      reverseWarn(`[ORDER_WS] Ignorando execução antiga: ${new Date(executionTime)} < ${new Date(executionStartTime)}`);
      return;
    }
    
    // ✅ NOVO: Validação de timestamp futuro (erro de clock)
    if (executionTime > Date.now() + 60000) {
      reverseWarn(`[ORDER_WS] Ignorando execução no futuro: ${new Date(executionTime)}`);
      return;
    }
    ...
    if (!sentOrders.has(orderId)) {
      ...
      reverseLog(`[ORDER_WS] Nova ordem rastreada via WS: ${orderId} (${orderStatus})`);
    } else {
      ...
      if (executionType === 'TRADE') {
        ...
        if (fillQty > 0) {
          ...
          if (!fillExists) {
            ...
            reverseLog(`[ORDER_WS] Fill recebido para ${orderId}: ${fillQty} @ ${fillPrice} (Trade ID: ${tradeId})`);
            ...
            if (Math.abs(totalFilled - orderInfo.executedQty) > 0.00000001) {
              reverseWarn(`[ORDER_WS] ⚠️ Inconsistência nos fills para ${orderId}: ${totalFilled} vs ${orderInfo.executedQty}`);
            }
          } else {
            reverseLog(`[ORDER_WS] Fill duplicado ignorado para ${orderId}: ${fillQty} @ ${fillPrice}`);
          }
        }
      }
      ...
      if (oldStatus !== orderStatus) {
        reverseLog(`[ORDER_WS] Status mudou para ${orderId}: ${oldStatus} → ${orderStatus}`);
      }
      
      if (oldExecutedQty !== orderInfo.executedQty) {
        const diff = orderInfo.executedQty - oldExecutedQty;
        reverseLog(`[ORDER_WS] Execução atualizada para ${orderId}: +${diff.toFixed(8)} (Total: ${orderInfo.executedQty})`);
      }
  ...
  try {
    ...
    reverseLog(`[LIMIT_ENTRY] ✅ Handler de WebSocket temporário registrado com ID: ${temporaryListenerId}`);
    ...
    reverseLog(`[LIMIT_ENTRY] Iniciando LIMIT MAKER para Sinal ID ${signal.id} (${signal.symbol}) na conta ${accountId}`);
    ...
    if (positionAlreadyExists) {
      reverseLog(`[LIMIT_ENTRY] ALERTA: Posição já existe para ${signal.symbol}. Cancelando.`);
      await connection.query(
    ...
    if (existingOrders.length > 0) {
      reverseLog(`[LIMIT_ENTRY] ⚠️ Já existem ${existingOrders.length} ordens ativas para este sinal. Cancelando...`);

      for (const order of existingOrders) {
        try {
          await cancelOrder(signal.symbol, order.id_externo, numericAccountId);
          reverseLog(`[LIMIT_ENTRY] Ordem ${order.id_externo} cancelada.`);
          ...
          reverseError(`[LIMIT_ENTRY] Erro ao cancelar ordem ${order.id_externo}:`, cancelError.message);
        }
      }
    ...
    reverseLog(`[LIMIT_ENTRY] Precisão obtida para ${signal.symbol}: qty=${quantityPrecision}, price=${pricePrecision}, step=${stepSize}`);
    ...
    if (signal.price && signal.price > 0) {
      ...
      reverseLog(`[LIMIT_ENTRY] Usando preço do sinal: ${currentPriceTrigger}`);
    } else if (signal.entry_price && signal.entry_price > 0) {
      currentPriceTrigger = parseFloat(signal.entry_price);
      reverseLog(`[LIMIT_ENTRY] Usando entry_price do sinal: ${currentPriceTrigger}`);
    } else {
      currentPriceTrigger = await rest.getPrice(signal.symbol, numericAccountId);
      reverseLog(`[LIMIT_ENTRY] Preço obtido via API: ${currentPriceTrigger}`);
    }
    ...
    reverseLog(`[LIMIT_ENTRY] Tamanho total calculado: ${totalEntrySize.toFixed(quantityPrecision)} ${signal.symbol}`);

    // ✅ VALIDAR QUANTIDADE TOTAL - MANTIDO
    reverseLog(`[LIMIT_ENTRY] Validando quantidade total de entrada: ${totalEntrySize}`);
    ...
    if (!entryValidation.isValid) {
      reverseWarn(`[LIMIT_ENTRY] ⚠️ Quantidade de entrada inválida: ${entryValidation.reason}`);
      ...
      if (adjustment.success) {
        reverseLog(`[LIMIT_ENTRY] ✅ Quantidade ajustada: ${totalEntrySize} → ${adjustment.adjustedQuantity}`);
        ...
        reverseError(`[LIMIT_ENTRY] ❌ ${errorMsg}`);
      ...
      reverseLog(`[LIMIT_ENTRY] ✅ Quantidade de entrada válida: ${totalEntrySize}`);
    }
    ...
    reverseLog(`[LIMIT_ENTRY] Iniciando WebSocket de profundidade para ${signal.symbol}`);
    ...
    depthWs = websockets.bookTicker(signal.symbol, (depthData, receivedAccountId) => {
      ...
      if (receivedAccountId && receivedAccountId !== accountId) {
        priceWSLog(`[LIMIT_ENTRY_DEPTH_WS] Dados recebidos para conta diferente: esperado ${accountId}, recebido ${receivedAccountId}`);
        return;
      }
      ...
      if (depthData && depthData.bestBid && depthData.bestAsk) {
        ...
        if (spreadPercent > 5) {
          priceWSLog(`[LIMIT_ENTRY_DEPTH_WS] Spread muito grande para ${signal.symbol}: ${spreadPercent.toFixed(2)}% - possível erro nos dados`);
          wsUpdateErrorCount++;
          return;
        }

        if (bid >= ask) {
          priceWSLog(`[LIMIT_ENTRY_DEPTH_WS] Dados inválidos para ${signal.symbol}: bid (${bid}) >= ask (${ask})`);
          wsUpdateErrorCount++;
          return;
        }
        ...
        if (Math.random() < 0.05) { // 5% de chance = ~1 a cada 20 mensagens
          //priceWSLog(`[LIMIT_ENTRY_DEPTH_WS] ${signal.symbol}: Bid=${bid.toFixed(pricePrecision)}, Ask=${ask.toFixed(pricePrecision)}, Spread=${spread.toFixed(pricePrecision)}`);
        }
      } else {
        wsUpdateErrorCount++;
        priceWSLog(`[LIMIT_ENTRY_DEPTH_WS] Dados de profundidade inválidos para ${signal.symbol}:`, depthData);
      }
    }, accountId);
    ...
    reverseLog(`[LIMIT_ENTRY] Aguardando dados do BookTicker WebSocket...`);
    while (wsRetryCount < MAX_RETRY_ATTEMPTS && !hasValidBookData) {
      if (currentBestBid !== null && currentBestAsk !== null &&
        ...
        reverseLog(`[LIMIT_ENTRY] ✅ Dados do BookTicker (WebSocket) válidos para ${signal.symbol} após ${wsRetryCount} tentativas. Bid=${currentBestBid}, Ask=${currentBestAsk}`);
        break;
      }

      wsRetryCount++;
      //reverseLog(`[LIMIT_ENTRY] Aguardando WebSocket... tentativa ${wsRetryCount}/${MAX_RETRY_ATTEMPTS}`);
    ...
    reverseLog(`[LIMIT_ENTRY] ✅ Dados WebSocket prontos. Iniciando loop de chasing...`);
    reverseLog(`[LIMIT_ENTRY] Configuração: tickSize=${tickSize}, totalEntrySize=${totalEntrySize.toFixed(quantityPrecision)}`);
    ...
    while (totalFilledSize < totalEntrySize && 
      ...
      try {
        ...
        for (const [orderId, orderInfo] of sentOrders.entries()) {
          if ((orderInfo.status === 'FILLED') &&
            ...
            if (orderInfo.executionTime >= executionStartTime) {
              ...
              reverseWarn(`[LIMIT_ENTRY] Ignorando fill antigo: ${orderId} (${new Date(orderInfo.executionTime)})`);
            }
          }
        }
        ...
        if (localFilledCount > 0) {
          ...
          reverseLog(`[LIMIT_ENTRY] ${localFilledCount} ordens sincronizadas via WebSocket local. Total: ${totalFilledSize.toFixed(quantityPrecision)}`);
        }
        ...
        if (apiFilledCount > 0) {
          totalFilledSize = partialFills.reduce((sum, pf) => sum + pf.qty, 0);
          reverseLog(`[LIMIT_ENTRY] ${apiFilledCount} ordens sincronizadas via rest. Total: ${totalFilledSize.toFixed(quantityPrecision)}`);
        }
        
        if (totalFilledSize >= totalEntrySize) {
          reverseLog(`[LIMIT_ENTRY] (Sync) Quantidade total atingida após sincronização. Finalizando...`);
          break;
        }
      } catch (checkError) {
        reverseError(`[LIMIT_ENTRY] Erro ao sincronizar ordens:`, checkError.message);
      }
      ...
      if (remainingSizeCurrentLoop <= 0) {
        reverseLog(`[LIMIT_ENTRY] Quantidade restante (${remainingSizeCurrentLoop.toFixed(quantityPrecision)}) zerada ou negativa. Saindo do loop de chasing.`);
        break;
      }
      ...
      if (!currentBestBid || !currentBestAsk || isDepthDataStale || wsUpdateErrorCount > 3) {
        ...
        priceWSLog(`[LIMIT_ENTRY] Dados do BookTicker (WebSocket) para ${signal.symbol} ${staleReason}. Tentativa ${chaseAttempts}/${MAX_CHASE_ATTEMPTS}. Última atualização WS: ${lastDepthUpdateTimestamp ? new Date(lastDepthUpdateTimestamp).toISOString() : 'N/A'}`);
        ...
        if (isWebSocketStale && chaseAttempts % 10 === 0) { // A cada 10 tentativas
          reverseWarn(`[LIMIT_ENTRY] ⚠️ WebSocket possivelmente desconectado. Tentando ressincronização...`);
      ...
      if (binanceSide === 'BUY') {
        ...
        if (candidatePrice < bestAsk) {
          ...
          reverseLog(`[LIMIT_ENTRY] BUY agressivo: dentro do spread a ${currentLocalMakerPrice.toFixed(pricePrecision)} (1 tick acima do bid)`);
        } else {
          ...
          if (currentLocalMakerPrice <= bestBid) {
            ...
            reverseLog(`[LIMIT_ENTRY] BUY: spread apertado, colocando no bestBid: ${currentLocalMakerPrice.toFixed(pricePrecision)}`);
          } else {
            reverseLog(`[LIMIT_ENTRY] BUY: spread apertado, ajustando para 1 tick abaixo do ask: ${currentLocalMakerPrice.toFixed(pricePrecision)}`);
          }
        }
      } else { // SELL
        ...
        if (candidatePrice > bestBid) {
          currentLocalMakerPrice = candidatePrice;
          reverseLog(`[LIMIT_ENTRY] SELL agressivo: dentro do spread a ${currentLocalMakerPrice.toFixed(pricePrecision)} (1 tick abaixo do ask)`);
        } else {
          ...
          if (currentLocalMakerPrice >= bestAsk) {
            ...
            reverseLog(`[LIMIT_ENTRY] SELL: spread apertado, colocando no bestAsk: ${currentLocalMakerPrice.toFixed(pricePrecision)}`);
          } else {
            reverseLog(`[LIMIT_ENTRY] SELL: spread apertado, ajustando para 1 tick acima do bid: ${currentLocalMakerPrice.toFixed(pricePrecision)}`);
          }
        }
      }
      ...
      reverseLog(`[LIMIT_ENTRY] Preço MAKER ${binanceSide}: ${currentLocalMakerPrice.toFixed(pricePrecision)} | Book: Bid=${bestBid.toFixed(pricePrecision)}, Ask=${bestAsk.toFixed(pricePrecision)}, Spread=${(bestAsk-bestBid).toFixed(pricePrecision)}`);
      ...
      if (activeOrderId) {
        ...
        try {
          ...
          reverseLog(`[LIMIT_ENTRY] 🔍 Status atual da ordem ${activeOrderId}: ${currentOrderStatus}`);
        } catch (statusError) {
          if (statusError.message && (statusError.message.includes('Order does not exist') || statusError.message.includes('Unknown order'))) {
            reverseLog(`[LIMIT_ENTRY] ❌ Ordem ${activeOrderId} não existe mais na corretora. Resetando activeOrderId.`);
            sentOrders.delete(activeOrderId);
            activeOrderId = null;
            currentOrderStatus = 'NOT_FOUND';
          } else {
            reverseError(`[LIMIT_ENTRY] Erro ao verificar status da ordem ${activeOrderId}:`, statusError.message);
        ...
        if (currentOrderStatus === 'CANCELED' || currentOrderStatus === 'NOT_FOUND' || currentOrderStatus === 'EXPIRED') {
          reverseLog(`[LIMIT_ENTRY] 🗑️ Ordem ${activeOrderId} com status ${currentOrderStatus}. Removendo do controle e criando nova ordem.`);
          sentOrders.delete(activeOrderId);
          activeOrderId = null;
          ...
          if (!wsOrderData) {
            reverseLog(`[LIMIT_ENTRY] Ordem ${activeOrderId} não encontrada no mapa WebSocket. Resetando activeOrderId.`);
            activeOrderId = null;
          } else {
          ...
          if (netFilledSinceLastCheck > 0) {
            ...
            totalFilledSize = partialFills.reduce((sum, pf) => sum + pf.qty, 0);
            reverseLog(`[LIMIT_ENTRY] 💰 Preenchimento detectado ${activeOrderId}: ${netFilledSinceLastCheck.toFixed(quantityPrecision)} @ ${fillPrice.toFixed(pricePrecision)}. Total: ${totalFilledSize.toFixed(quantityPrecision)}`);
          }            if (effectiveStatus === 'FILLED') {
              reverseLog(`[LIMIT_ENTRY] ✅ Ordem ${activeOrderId} totalmente preenchida.`);
              sentOrders.delete(activeOrderId);
              activeOrderId = null;
              if (totalFilledSize >= totalEntrySize) {
                reverseLog(`[LIMIT_ENTRY] 🎯 Quantidade total atingida após FILLED.`);
                break;
              }
              ...
              if (shouldEditOrder) {
                reverseLog(`[LIMIT_ENTRY] ⚡ Ordem ${activeOrderId} parcialmente preenchida, mas preço mudou significativamente (${priceDifference.toFixed(pricePrecision)} > ${tickSize}). Cancelando e recriando...`);
                
                try {
                  await cancelOrder(signal.symbol, activeOrderId, numericAccountId);
                  reverseLog(`[LIMIT_ENTRY] ✅ Ordem parcial ${activeOrderId} cancelada.`);
                  ...
                  reverseError(`[LIMIT_ENTRY] ❌ Erro ao cancelar ordem parcial ${activeOrderId}:`, cancelError.message);
                  // Se não conseguir cancelar, resetar mesmo assim para evitar loop
                  sentOrders.delete(activeOrderId);
                  activeOrderId = null;
                }
              } else {
                reverseLog(`[LIMIT_ENTRY] ⏸️ Ordem ${activeOrderId} parcialmente preenchida, mas preço ainda adequado (diferença: ${priceDifference.toFixed(pricePrecision)}). Mantendo ordem.`);
              }
              ...
              if (shouldEditOrder) {
                reverseLog(`[LIMIT_ENTRY] ✏️ Editando ordem ${activeOrderId}: ${orderPriceOnExchange.toFixed(pricePrecision)} → ${currentLocalMakerPrice.toFixed(pricePrecision)} (diferença: ${priceDifference.toFixed(pricePrecision)})`);
                ...
                try {
                  ...
                  if (editResponse && (editResponse.orderId || editResponse.data?.orderId)) {
                    ...
                    if (String(newOrderId) !== String(activeOrderId)) {
                      reverseLog(`[LIMIT_ENTRY] 🔄 Ordem recriada: ${activeOrderId} → ${newOrderId}`);
                    ...
                    reverseLog(`[LIMIT_ENTRY] ✅ Ordem editada com sucesso: ${activeOrderId}`);
                  } else {
                    reverseWarn(`[LIMIT_ENTRY] ⚠️ Resposta inválida ao editar ordem:`, editResponse);
                  }
                } catch (editError) {
                  reverseError(`[LIMIT_ENTRY] ❌ Erro ao editar ordem ${activeOrderId}:`, editError.message);
                  ...
                  try {
                    ...
                    sentOrders.delete(activeOrderId);
                    activeOrderId = null;
                    reverseLog(`[LIMIT_ENTRY] 🗑️ Ordem cancelada após erro de edição.`);
                  } catch (cancelError) {
                    reverseError(`[LIMIT_ENTRY] ❌ Erro ao cancelar ordem após falha de edição:`, cancelError.message);
                ...
                reverseLog(`[LIMIT_ENTRY] ✅ Ordem ${activeOrderId} já no preço adequado (${orderPriceOnExchange.toFixed(pricePrecision)}). Diferença: ${priceDifference.toFixed(pricePrecision)}`);
              }
            } else {
              reverseLog(`[LIMIT_ENTRY] ℹ️ Ordem ${activeOrderId} em status: ${effectiveStatus}`);
            }
          }
        }
      }
      ...
      if (!activeOrderId && totalFilledSize < totalEntrySize) { 
        ...
        if (newOrderQty <= 0) { 
          reverseLog("[LIMIT_ENTRY] Quantidade para nova ordem é zero ou negativa. Saindo do loop de chasing."); 
          break; 
        }
        
        try {
          reverseLog(`[LIMIT_ENTRY] Enviando NOVA LIMIT ${signal.symbol}: ${binanceSide} ${newOrderQty.toFixed(quantityPrecision)} @ ${currentLocalMakerPrice.toFixed(pricePrecision)} (MAKER GARANTIDO - ${binanceSide === 'BUY' ? 'bestBid-1tick' : 'bestAsk+1tick'})`);
          reverseLog(`[LIMIT_ENTRY] 📊 Book atual: Bid=${bestBid.toFixed(pricePrecision)}, Ask=${bestAsk.toFixed(pricePrecision)}, Spread=${spread.toFixed(pricePrecision)}`);
          ...
          if (orderResponse.status === 'REJECTED_POST_ONLY' || 
            ...
            rejectionAttempts++;
            reverseLog(`[LIMIT_ENTRY] ⚠️ Ordem rejeitada como maker (${rejectionAttempts}/${MAX_REJECTION_ATTEMPTS}): ${orderResponse.status || orderResponse.info?.msg || orderResponse.msg}`);
            reverseLog(`[LIMIT_ENTRY] ❌ ERRO CRÍTICO: A lógica MAKER GARANTIDA deveria SEMPRE ser aceita como maker!`);
            reverseLog(`[LIMIT_ENTRY] 📊 Debug - BestBid: ${bestBid.toFixed(pricePrecision)}, BestAsk: ${bestAsk.toFixed(pricePrecision)}, Preço enviado: ${currentLocalMakerPrice.toFixed(pricePrecision)}, Side: ${binanceSide}`);
            
            if (rejectionAttempts >= MAX_REJECTION_ATTEMPTS) {
              reverseLog(`[LIMIT_ENTRY] ❌ Muitas rejeições (${rejectionAttempts}). Algo está errado com a lógica MAKER GARANTIDA para sinal ${signal.id}`);
            ...
            reverseLog(`[LIMIT_ENTRY] 🔄 Aguardando próxima iteração com dados frescos do WebSocket...`);
            await new Promise(resolve => setTimeout(resolve, 1000));
            continue;
          }
          ...
          orderPlacedOrEditedThisIteration = true;
          reverseLog(`[LIMIT_ENTRY] Nova LIMIT criada: ID ${activeOrderId}`);
          ...
          reverseError(`[LIMIT_ENTRY] Erro ao criar NOVA LIMIT:`, newOrderError.message);
          await new Promise(resolve => setTimeout(resolve, 1000)); 
          continue;
        }
      }
      ...
      if (orderPlacedOrEditedThisIteration && activeOrderId) {
        reverseLog(`[LIMIT_ENTRY] Aguardando execução/status da ordem ${activeOrderId} por ${EDIT_WAIT_TIMEOUT_MS}ms...`);
        ...
        if (netFilledInWait > 0) {
          ...
          totalFilledSize = partialFills.reduce((sum, pf) => sum + pf.qty, 0);
          reverseLog(`[LIMIT_ENTRY] Preenchimento (após wait) ${activeOrderId}: ${netFilledInWait.toFixed(quantityPrecision)} @ ${fillPrice.toFixed(pricePrecision)}. Total: ${totalFilledSize.toFixed(quantityPrecision)}`);
        }

        if (orderWaitResult.status === 'FILLED') {
          reverseLog(`[LIMIT_ENTRY] Ordem ${activeOrderId} FILLED (após wait).`);
          activeOrderId = null; 
          if (totalFilledSize >= totalEntrySize) {
            reverseLog(`[LIMIT_ENTRY] Quantidade total atingida após wait.`);
            break;
          }
        } else if (orderWaitResult.status === 'PARTIALLY_FILLED') {
          reverseLog(`[LIMIT_ENTRY] Ordem ${activeOrderId} parcialmente preenchida após wait.`);
        } else {
          reverseLog(`[LIMIT_ENTRY] Ordem ${activeOrderId} ainda em ${orderWaitResult.status} após wait.`);
        }
      }
      
      if (totalFilledSize >= totalEntrySize) {
        reverseLog(`[LIMIT_ENTRY] Quantidade total (${totalEntrySize.toFixed(quantityPrecision)}) alcançada.`);
        break; 
      }
    ...
    if (totalFilledSize < totalEntrySize) {
      reverseLog(`[LIMIT_ENTRY] 📊 Chasing encerrado. Preenchido: ${totalFilledSize.toFixed(quantityPrecision)}/${totalEntrySize.toFixed(quantityPrecision)} (${((totalFilledSize/totalEntrySize)*100).toFixed(1)}%)`);
      reverseLog(`[LIMIT_ENTRY] 📊 Motivos: Timeout: ${Date.now() - executionStartTime >= CHASE_TIMEOUT_MS}, Tentativas: ${chaseAttempts >= MAX_CHASE_ATTEMPTS}`);
      ...
      if (remainingToFillMarket > minMarketOrderThreshold) {
        reverseLog(`[LIMIT_ENTRY] 🚀 Tentando preencher restante (${remainingToFillMarket.toFixed(quantityPrecision)}) com ordem MARKET...`);
        
        // CANCELAR ORDEM ATIVA ANTES DE MARKET
        if (activeOrderId) {
          try {
            reverseLog(`[LIMIT_ENTRY] 🗑️ Cancelando ordem ativa ${activeOrderId} antes da MARKET...`);
            await cancelOrder(signal.symbol, activeOrderId, numericAccountId);
            await new Promise(resolve => setTimeout(resolve, 1000)); // Aguardar 1 segundo
            activeOrderId = null;
            reverseLog(`[LIMIT_ENTRY] ✅ Ordem ativa cancelada com sucesso.`);
          } catch (cancelError) {
            reverseError(`[LIMIT_ENTRY] ❌ Erro ao cancelar ordem ativa antes da MARKET:`, cancelError.message);
          }
        }
        ...
        try {
          ...
          if (!marketValidation.isValid) {
            reverseWarn(`[LIMIT_ENTRY] ⚠️ Quantidade MARKET inválida: ${marketValidation.reason}`);
            ...
            if (marketAdjustment.success) {
              ...
              reverseLog(`[LIMIT_ENTRY] ✅ Quantidade MARKET ajustada: ${remainingToFillMarket} → ${finalMarketQty}`);
            } else {
              reverseError(`[LIMIT_ENTRY] ❌ Impossível ajustar quantidade MARKET: ${marketAdjustment.error}`);
              throw new Error(`Quantidade MARKET inválida: ${marketAdjustment.error}`);
            }
          }
          
          reverseLog(`[LIMIT_ENTRY] 🚀 Enviando ordem MARKET: ${binanceSide} ${finalMarketQty.toFixed(quantityPrecision)} ${signal.symbol}`);
          ...
          if (marketOrderResponseForDb && marketOrderResponseForDb.orderId) {
            reverseLog(`[LIMIT_ENTRY] ✅ Ordem MARKET criada: ${marketOrderResponseForDb.orderId}`);
            ...
            if (marketWaitResult && marketWaitResult.status === 'FILLED') {
              ...
              if (marketFilledQty > 0) {
                ...
                reverseLog(`[LIMIT_ENTRY] 💰 MARKET executada: ${marketFilledQty.toFixed(quantityPrecision)} @ ${marketAvgPrice.toFixed(pricePrecision)}. Total final: ${totalFilledSize.toFixed(quantityPrecision)}`);
              }
            } else {
              reverseWarn(`[LIMIT_ENTRY] ⚠️ Ordem MARKET não foi totalmente executada: ${marketWaitResult?.status || 'UNKNOWN'}`);
            }              
              ...
              reverseError(`[LIMIT_ENTRY] ❌ Falha ao criar ordem MARKET. Resposta: ${JSON.stringify(marketOrderResponseForDb)}`);
            }
          } catch (marketError) {
            reverseError(`[LIMIT_ENTRY] ❌ Erro na ordem MARKET:`, marketError.response?.data || marketError.message);
          }
        } else {
          reverseLog(`[LIMIT_ENTRY] ⏭️ Quantidade restante (${remainingToFillMarket.toFixed(quantityPrecision)}) muito pequena para MARKET. Prosseguindo...`);
        }
      }
    ...
    if (partialFills.length > 0) {
      ...
      reverseLog(`[LIMIT_ENTRY] 📊 Posição criada no buffer: ${signal.symbol} (${totalFilledSize.toFixed(quantityPrecision)} @ ${averageEntryPrice.toFixed(pricePrecision)})`);
      ...
      reverseWarn(`[LIMIT_ENTRY] averageEntryPrice não pôde ser calculado a partir de partialFills (total preenchido: ${totalFilledSize}), usando currentPriceTrigger como fallback: ${averageEntryPrice}`);
    }

    // ✅ MARCAR QUE TODAS AS ORDENS DE ENTRADA FORAM ENVIADAS
    allOrdersSent = true;
    reverseLog(`[LIMIT_ENTRY] 🚀 Todas as ordens de entrada foram enviadas. Iniciando criação de SL/TP/RPs...`);
    ...
    reverseLog(`[LIMIT_ENTRY] ✅ Entrada executada: ${totalFilledSize.toFixed(quantityPrecision)}/${totalEntrySize.toFixed(quantityPrecision)} (${(fillRatio * 100).toFixed(1)}%)`);
    reverseLog(`[LIMIT_ENTRY] 📡 Aguardando confirmação da posição via webhook para criar SL/TP/RPs...`);

    // ✅ SUBSTITUIR POR LOG APENAS:
    reverseLog(`[LIMIT_ENTRY] 📊 Ordens de entrada executadas: ${partialFills.length} fills`);
    partialFills.forEach((fill, index) => {
      reverseLog(`[LIMIT_ENTRY]   Fill ${index + 1}: ${fill.qty.toFixed(quantityPrecision)} @ ${fill.price.toFixed(pricePrecision)} (ID: ${fill.orderId})`);
    });
    reverseLog(`[LIMIT_ENTRY] � Dados armazenados no buffer, aguardando inserção no banco...`);
    ...
    if (isEntryReallyComplete) {
      ...
      reverseLog(`[LIMIT_ENTRY] 📋 Preços configurados:`);
      reverseLog(`  - SL: ${slPriceVal || 'N/A'}`);
      reverseLog(`  - TP1: ${targetPrices.tp1 || 'N/A'}`);
      reverseLog(`  - TP2: ${targetPrices.tp2 || 'N/A'}`);
      reverseLog(`  - TP3: ${targetPrices.tp3 || 'N/A'}`);
      reverseLog(`  - TP4: ${targetPrices.tp4 || 'N/A'}`);
      reverseLog(`  - TP5: ${targetPrices.tp5 || 'N/A'}`);
      ...
      reverseLog(`[LIMIT_ENTRY] 📊 Criando RPs com percentuais: ${reductionPercentages.map(p => (p*100)+'%').join(', ')}`);
      ...
      if (targetPrices.tp5 && targetPrices.tp5 > 0) {
        try {
          ...
          reverseError(`[LIMIT_ENTRY] ❌ Erro ao enviar TAKE_PROFIT_MARKET TP5 para corretora:`, tp5Error.response?.data || tp5Error.message);
        }
      }
    ...
    await connection.commit();
    reverseLog(`[LIMIT_ENTRY] ✅ Transação COMMITADA com sucesso para sinal ${signal.id}`);
    ...
    if (insertResult.success) {
      ...
      reverseLog(`[LIMIT_ENTRY] ✅ Dados inseridos no banco com sucesso. Position ID: ${positionId}`);
    } else {
      reverseError(`[LIMIT_ENTRY] ❌ Falha ao inserir dados do buffer no banco`);
    }
    ...
    try {
      ...
      reverseLog(`[TELEGRAM_DISPATCHER] 🕐 ${timestamp} | ARQUIVO: reverse.js | AÇÃO: Disparando mensagem de ENTRADA EXECUTADA`);
      reverseLog(`[TELEGRAM_DISPATCHER] 📋 Dados: Conta=${accountId}, Symbol=${signal.symbol}, Qtd=${totalFilledSize}, Preço=${averageEntryPrice}, Valor=${totalValue.toFixed(2)}`);
      reverseLog(`[TELEGRAM_DISPATCHER] 🎯 Signal ID: ${signal.id}, Position ID: ${positionId || 'ainda não definido'}`);
      
      const telegramResult = await formatEntryMessageWithPrecision(signal, totalFilledSize, averageEntryPrice, totalValue, accountId);
      
      reverseLog(`[TELEGRAM_DISPATCHER] 📤 Resultado do envio:`, JSON.stringify(telegramResult, null, 2));
      
      if (telegramResult && telegramResult.success) {
        reverseLog(`[TELEGRAM_DISPATCHER] ✅ ${timestamp} | reverse.js | Mensagem de entrada enviada com SUCESSO para conta ${accountId}`);
        ...
        if (telegramResult.messageId) {
          ...
          reverseLog(`[TELEGRAM_DISPATCHER] 📨 Message ID (result.message_id): ${messageId}`);
        } else {
          reverseWarn(`[TELEGRAM_DISPATCHER] ⚠️ Nenhum messageId encontrado na resposta`);
        }
        ...
        if (messageId) {
          ...
          reverseLog(`[TELEGRAM_DISPATCHER] 📋 Message ID salvo no banco: ${messageId} para sinal ${signal.id}`);
        }
      } else {
        reverseWarn(`[TELEGRAM_DISPATCHER] ⚠️ ${timestamp} | reverse.js | FALHA ao enviar mensagem de entrada`);
        reverseWarn(`[TELEGRAM_DISPATCHER] 🔍 Erro:`, telegramResult?.error || 'Erro desconhecido');
        reverseWarn(`[TELEGRAM_DISPATCHER] 🔍 Resposta completa:`, telegramResult);
      }
    } catch (telegramError) {
      const timestamp = new Date().toLocaleString('pt-BR');
      reverseError(`[TELEGRAM_DISPATCHER] ❌ ${timestamp} | reverse.js | ERRO CRÍTICO ao enviar notificação de entrada:`, telegramError.message);
      reverseError(`[TELEGRAM_DISPATCHER] 📋 Stack trace:`, telegramError.stack);
    }

    // ✅ REMOVER LÓGICA DE BUSCA DE POSIÇÃO - AGORA USAMOS O POSITION_ID DO BUFFER
    if (positionId) {
      reverseLog(`[LIMIT_ENTRY] 🔗 Sinal ${signal.id} já vinculado à posição ${positionId}`);
    } else {
      reverseWarn(`[LIMIT_ENTRY] ⚠️ Position ID não disponível para sinal ${signal.id}`);
    }
  } catch (error) { // This is the catch block for the main try
    const originalErrorMessage = error.message || String(error);
    reverseError(`[LIMIT_ENTRY] ERRO FATAL DURANTE ENTRADA (Sinal ID ${signal.id}): ${originalErrorMessage}`, error.stack || error);

    // ✅ NOTIFICAÇÃO TELEGRAM DE ERRO
    try {
      const timestamp = new Date().toLocaleString('pt-BR');
      reverseLog(`[TELEGRAM_DISPATCHER] 🕐 ${timestamp} | ARQUIVO: reverse.js | AÇÃO: Disparando mensagem de ERRO`);
      reverseLog(`[TELEGRAM_DISPATCHER] 📋 Dados: Conta=${accountId}, Signal=${signal.id}, Erro=${originalErrorMessage}`);
      ...
      reverseLog(`[TELEGRAM_DISPATCHER] � Resultado do envio de erro:`, JSON.stringify(result, null, 2));
      
      if (result && result.success) {
        reverseLog(`[TELEGRAM_DISPATCHER] ✅ ${timestamp} | reverse.js | Mensagem de erro enviada com SUCESSO via Telegram`);
      } else {
        reverseWarn(`[TELEGRAM_DISPATCHER] ⚠️ ${timestamp} | reverse.js | FALHA ao enviar mensagem de erro:`, result?.error || 'Erro desconhecido');
      }
    } catch (telegramError) {
      const timestamp = new Date().toLocaleString('pt-BR');
      reverseError(`[TELEGRAM_DISPATCHER] ❌ ${timestamp} | reverse.js | ERRO CRÍTICO ao enviar notificação de erro:`, telegramError.message);
    }

    // CANCELAR ORDEM ATIVA SE EXISTIR USANDO rest.JS
    if (activeOrderId) {
      try {
        reverseLog(`[LIMIT_ENTRY] (Catch Principal) Tentando cancelar ordem ativa ${activeOrderId} antes do rollback.`);
        
        // ✅ USAR cancelOrder DO rest.JS
        await cancelOrder(signal.symbol, activeOrderId, numericAccountId); 
        
        reverseLog(`[LIMIT_ENTRY] (Catch Principal) Ordem ${activeOrderId} cancelada com sucesso.`);
      } catch (cancelErrOnCatch) { 
        reverseError(`[LIMIT_ENTRY] (Catch Principal) Erro ao cancelar ordem ${activeOrderId}:`, cancelErrOnCatch.message); 
      }
    }
    ...
    if (positionBuffer.has(signal.id)) {
      ...
      reverseLog(`[LIMIT_ENTRY] 🧹 Buffer de posição limpo para signal ${signal.id}`);
    }
    ...
    if (signalOrderKeys.length > 0) {
      reverseLog(`[LIMIT_ENTRY] 🧹 ${signalOrderKeys.length} ordens removidas do buffer para signal ${signal.id}`);
    }

    if (connection) { 
      try { 
        await connection.rollback(); 
        reverseLog(`[LIMIT_ENTRY] (Catch Principal) ROLLBACK da transação principal efetuado para Sinal ${signal.id}.`); 
      } catch (rbErr) { 
        reverseError(`[LIMIT_ENTRY] (Catch Principal) Erro CRÍTICO ao efetuar ROLLBACK para Sinal ${signal.id}:`, rbErr); 
      }
    }
    ...
    try {
      ...
      reverseError(`[LIMIT_ENTRY] (Catch Principal) Erro ao atualizar status do sinal para ERROR no DB:`, updateError); 
    }
    ...
    reverseLog(`[LIMIT_ENTRY] 🧹 Finalizando execução para sinal ${signal.id}. Iniciando limpeza...`);

    // ✅ REMOVER HANDLER TEMPORÁRIO DO WEBSOCKET USANDO O NOVO SISTEMA
    websockets.off('orderUpdate', temporaryListenerId, accountId);
    reverseLog(`[LIMIT_ENTRY] ✅ Handler de WebSocket temporário removido.`);

    // Parar o websocket de profundidade
    if (depthWs && typeof depthWs.close === 'function') {
      try {
        depthWs.close(1000, 'Execution completed');
        reverseLog(`[LIMIT_ENTRY] ✅ WebSocket de profundidade parado com sucesso.`);
      } catch (wsCloseError) {
        reverseError(`[LIMIT_ENTRY] Erro ao fechar WebSocket de profundidade:`, wsCloseError.message);
      }
    }
    ...
    if (connection) {
      try {
        connection.release();
        reverseLog(`[LIMIT_ENTRY] Conexão de banco liberada para sinal ${signal?.id || 'unknown'}`);
      } catch (releaseError) {
        reverseError(`[LIMIT_ENTRY] Erro ao liberar conexão:`, releaseError.message);
      }
    }
  }
...
async function waitForOrderExecution(symbol, orderId, maxWaitMs = 3000, accountId, sentOrdersMap) {
  ...
  try {
    reverseLog(`[WAIT_ORDER] Aguardando execução da ordem ${orderId} por ${maxWaitMs}ms...`);
    ...
    if (sentOrdersMap && sentOrdersMap.has(orderId)) {
      ...
      if (orderInfo.status === 'FILLED' || orderInfo.status === 'PARTIALLY_FILLED') {
        reverseLog(`[WAIT_ORDER] ✅ Ordem ${orderId} já confirmada via WebSocket local: ${orderInfo.status}`);
        return {
    ...
    try {
      ...
      if (orderStatus && (orderStatus.status === 'FILLED' || orderStatus.status === 'PARTIALLY_FILLED')) {
        reverseLog(`[WAIT_ORDER] ✅ Ordem ${orderId} confirmada via REST: ${orderStatus.status}`);
        ...
        return orderStatus;
           ...
           reverseLog(`[WAIT_ORDER] ⚠️ Erro REST (pode ser apenas atraso): ${restError.message}`);
    }
    ...
    while (checks < maxChecks) {
      ...
      if (sentOrdersMap && sentOrdersMap.has(orderId)) {
        const orderInfo = sentOrdersMap.get(orderId);
        if (orderInfo.status === 'FILLED' || orderInfo.status === 'PARTIALLY_FILLED') {
          reverseLog(`[WAIT_ORDER] ✅ Ordem ${orderId} confirmada via WebSocket durante espera: ${orderInfo.status}`);
          return {
            status: orderInfo.status,
            executedQty: orderInfo.executedQty.toString(),
            avgPrice: (orderInfo.avgPrice || orderInfo.price).toString(),
      ...
      if (checks % 10 === 0) {
        try {
          const orderStatus = await getOrderStatus(symbol, orderId, accountId);
          if (orderStatus && (orderStatus.status === 'FILLED' || orderStatus.status === 'PARTIALLY_FILLED')) {
            reverseLog(`[WAIT_ORDER] ✅ Ordem ${orderId} confirmada via REST durante espera: ${orderStatus.status}`);
            return orderStatus;
          }
        } catch (restError) {
          reverseLog(`[WAIT_ORDER] ⚠️ Erro REST durante espera: ${restError.message}`);
        }
      }
    }
    
    reverseWarn(`[WAIT_ORDER] ⏰ Tempo limite atingido (${maxWaitMs}ms) para a ordem ${orderId}. Status atual pode ser: FILLED, PARTIALLY_FILLED ou ainda NEW.`);
    return { status: 'PENDING' };
  } catch (error) {
    reverseError(`[WAIT_ORDER] ERRO:`, error.message);
    return { status: 'ERROR' };
  }
}
  ...
  try {
    ...
    for (const order of activeOrders) {
      ...
      if (!sentOrdersMap.has(orderId)) {
        reverseWarn(`[GHOST_ORDER] ⚠️ Ordem não rastreada encontrada: ${orderId} (${order.status})`);
        ...
        reverseLog(`[GHOST_ORDER] ✅ Ordem fantasma ${orderId} adicionada ao rastreamento`);
      }
    }
  } catch (error) {
    reverseError(`[GHOST_ORDER] Erro na detecção: ${error.message}`);
  }
}
...
async function resyncAfterReconnection(symbol, executionStartTime, accountId, sentOrdersMap) {
  reverseLog(`[RESYNC] Ressincronizando após reconexão do WebSocket...`);
  ...
  try {
      ...
      if (order.updateTime >= executionStartTime) {
        ...
        if (sentOrdersMap.has(orderId)) {
          ...
          if (localOrder.lastUpdateTime < order.updateTime) {
            ...
            resyncCount++;
            reverseLog(`[RESYNC] Ordem ${orderId} atualizada: ${order.status} (exec: ${order.executedQty})`);
          }
        }
      }
    }
    ...
    if (resyncCount > 0) {
      reverseLog(`[RESYNC] ✅ ${resyncCount} ordens ressincronizadas com sucesso`);
    } else {
      reverseLog(`[RESYNC] ℹ️ Nenhuma atualização necessária durante ressincronização`);
    }
  } catch (error) {
    reverseError(`[RESYNC] Erro na ressincronização: ${error.message}`);
  }
}
...
async function syncOrderWithLock(orderId, symbol, accountId, sentOrdersMap, executionStartTime, partialFills, binanceSide) {
  if (orderLocks.has(orderId)) {
    reverseLog(`[LOCK] Aguardando lock para ordem ${orderId}...`);
  ...
  const lockPromise = (async () => {
    try {
      ...
      reverseLog(`[LOCK] Lock liberado para ordem ${orderId}`);
    }
  })();
...
async function syncAndProcessSingleOrder(orderId, symbol, accountId, sentOrdersMap, executionStartTime, partialFills, binanceSide) {
    reverseLog(`[SYNC_ORDER] Sincronizando ordem ${orderId}...`);
    let filledQty = 0;
    ...
    try {
            ...
            if (orderStatus.status === 'FILLED' && orderStatus.side === binanceSide) {
                ...
                if (!alreadyFilled) {
                    ...
                    if (filledQty > 0) {
                        ...
                        reverseLog(`[SYNC_ORDER] ✅ Ordem ${orderId} confirmada como FILLED. Qtd: ${filledQty}`);
        ...
        reverseError(`[SYNC_ORDER] Erro ao sincronizar ordem ${orderId}: ${error.message}`);
    }
...
async function getAvailableBalance(accountId) {
    try {
        reverseLog(`[LIMIT_ENTRY] Obtendo saldo_base_calculo para conta ${accountId}...`);
        ...
        reverseLog(`[LIMIT_ENTRY] ✅ saldo obtido: ${saldoBaseCalculoFuturos} USDT`);
        return saldoBaseCalculoFuturos;

    } catch (error) {
        reverseError(`[LIMIT_ENTRY] Erro ao obter saldo_base_calculo para conta ${accountId}:`, error.message);
    }
}
...
function calculateOrderSize(availableBalance, capitalPercentage, entryPrice, leverage, stepSize, quantityPrecision) {
    ...
    if (stepSize <= 0) {
        reverseWarn(`[MONITOR] StepSize inválido (${stepSize}), usando precisão para cálculo.`);
    ...
    reverseLog(`[MONITOR] Cálculo baseado em saldo_base_calculo:`);
    reverseLog(`[MONITOR]   - saldo_base_calculo: ${availableBalance.toFixed(2)} USDT`);
    reverseLog(`[MONITOR]   - formatado: ${formattedSize}`);
