...
/**
 * Função auxiliar para retry automático em operações de banco de dados
 * @param {Function} operation - Função que executa a operação
 * @param {number} maxRetries - Número máximo de tentativas (padrão: 10)
 * @param {number} delay - Delay entre tentativas em ms (padrão: 1000ms)
 * @param {string} operationName - Nome da operação para logs
 * @returns {Promise<any>} - Resultado da operação
 */
async function retryDatabaseOperation(operation, maxRetries = 10, delay = 1000, operationName = 'Database Operation') {
  let lastError = null;
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      // Antes de cada tentativa, aguardar um pequeno delay aleatório
      // para reduzir chances de múltiplos processos competirem pelos mesmos locks
      if (attempt > 1) {
        const jitter = Math.floor(Math.random() * 500); // Adiciona até 500ms de jitter
      ...
      if (attempt > 1) {
        console.log(`[RETRY] ✅ ${operationName} bem-sucedida na tentativa ${attempt}/${maxRetries}`);
      }
      ...
      const isRetryableError = 
        ...
        error.message.includes('ER_OUTOFMEMORY') ||
        error.message.includes('Server closed the connection') ||
        // Códigos de erro específicos MySQL
      ...
      // Se for erro recuperável e não excedeu o número máximo de tentativas
      if (isRetryableError && attempt < maxRetries) {
        // Exponential backoff com jitter
        const baseDelay = delay * Math.pow(1.5, attempt - 1);
        const jitter = Math.floor(Math.random() * (baseDelay * 0.2)); // 20% de jitter
        const backoffDelay = baseDelay + jitter;
        
        console.warn(`[RETRY] ⚠️ ${operationName} falhou na tentativa ${attempt}/${maxRetries}: ${error.message}`);
        console.warn(`[RETRY] 🔄 Tentando novamente em ${Math.round(backoffDelay)}ms (attempt ${attempt}/${maxRetries})...`);
        
        // Se for erro de lock, forçar um delay maior
        if (error.message.includes('Lock wait timeout') || error.code === 'ER_LOCK_WAIT_TIMEOUT') {
          console.warn(`[RETRY] 🔒 Erro de lock detectado, aumentando delay...`);
          await new Promise(resolve => setTimeout(resolve, backoffDelay + 2000));
        } else {
      ...
      // Se não é erro recuperável ou excedeu tentativas, relança o erro
      console.error(`[RETRY] ❌ ${operationName} falhou definitivamente após ${attempt} tentativas: ${error.message}`);
      throw error;
    }
  }
...
async function moveOrdersToHistory(accountId) {
  try {
    ...
    }, 10, 1000, `Move Orders to History (conta ${accountId})`);
...
async function checkSingleOrderStatus(orderId, accountId) {
  try {
    return await retryDatabaseOperation(async () => {
      ...
      return result;
    }, 10, 1000, `Check Single Order Status (ordem ${orderId})`);
  ...
  try {
    return await retryDatabaseOperation(async () => {
     ...
     // sendTelegramMessage(message);
      ...
      return 1;
    }, 10, 1000, `Move Position to History (posição ${positionId})`);
...
async function checkAndCloseWebsocket(accountId) {
  try {
    return await retryDatabaseOperation(async () => {
      ...
      return closedCount;
    }, 10, 1000, `Check and Close WebSocket (conta ${accountId})`);
...
async function movePositionToHistory(db, positionId, status = 'CLOSED', reason = 'Movida automaticamente', accountId) {
  try {
    ...
    }, 10, 1000, `Move Position to History Complex (posição ${positionId})`);
