    ...
    for (const exchangePos of exchangePositions) {
      ...
      if (!dbPos) {
        ...
        try {
          await db.query(`
            UPDATE posicoes 
            SET preco_corrente = $1, data_hora_ultima_atualizacao = CURRENT_TIMESTAMP
            WHERE id = $2
    ...
    for (const dbPos of dbPositions) {
      ...
      if (!exchangePos) {
        ...
        try {
          await db.query(`
            UPDATE posicoes 
            SET status = 'CLOSED', data_hora_fechamento = CURRENT_TIMESTAMP
            WHERE id = $1
          `, [dbPos.id]);
    ...
    for (const symbol of symbols) {
      try {
        ...
        for (const order of openOrders) {
          ...
          const result = await db.query(`SELECT id_externo, status, data_hora_criacao FROM ordens WHERE id_externo = $1 AND conta_id = $2`, [order.orderId, accountId]
          );

          if (existing.rows.length === 0) {
            // ✅ INSERIR ORDEM NOVA
            await db.query(`INSERT INTO ordens 
            ...
            if (currentStatus !== order.status) {
              await db.query(`UPDATE ordens SET status = $1, last_update = CURRENT_TIMESTAMP WHERE id_externo = $2 AND conta_id = $3`, [order.status, order.orderId, accountId]
              );
          ...
          if (finalizedStatuses.includes(order.status)) {
            ...
            if (result.rows.length > 0) {
              ...
              const updateResult = await db.query(`UPDATE ordens SET id_posicao = $1 WHERE id_externo = $2 AND conta_id = $3 AND (id_posicao IS NULL OR id_posicao != $4)`, [posId, order.orderId, accountId, posId]
              );
...
async function logOpenPositionsAndOrdersVisual(accountId) {
  try {
    ...
    const result = await db.query(`
      ...
      FROM posicoes WHERE status = 'OPEN' AND conta_id = $1
    `, [accountId]);
...
async function createMissingOrdersForPosition(accountId, position, latestSignal) {
  ...
  try {
    ...
    if (createdOrders.errors.rows.length > 0) {
    ...
    for (const position of exchangePositions) {
      try {
        ...
        if (openOrders.rows.length > 0) {
          console.log(`[ORPHAN_DETECTION] 📋 ${symbol}: tem ${openOrders.rows.length} ordens abertas, não é órfã`);
          continue;
        }
        ...
        if (dbPositionsResult.rows.length === 0) {
          ...
          console.log(`[ORPHAN_DETECTION] ✅ ${symbol}: encontrada no banco (ID: ${dbPositionsResult.rows[0].id}), verificando ordens...`);
          ...
          if (protectionOrders.rows[0].count > 0) {
            console.log(`[ORPHAN_DETECTION] ✅ ${symbol}: tem ${protectionOrders.rows[0].count} ordens de proteção, tudo ok`);
            continue;
          } else {
