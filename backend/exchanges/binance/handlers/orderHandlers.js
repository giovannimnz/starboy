...
async function handleOrderUpdate(accountId, orderUpdateData, db = null) {
  try {
    ...
    const result = await connection.query(`SELECT * FROM ordens WHERE id_externo = $1 AND simbolo = $2 AND conta_id = $3`, [orderId, symbol, accountId]
    );

    const orderExists = existingOrders.rows.length > 0;
    ...
    if (orderExists) {
      await updateExistingOrder(connection, orderData, accountId, existingOrders.rows[0]);
    }
  ...
  try {
    ...
    if (existingOrder) {
      ...
      while (tradeUpdateTries < 1000) {
        try {
          await connection.query(`UPDATE ordens SET 
             status = $1, 
             ...
             commission_asset = $5,
             trade_id = $6,
             dados_originais_ws = $7,
             last_update = CURRENT_TIMESTAMP
             WHERE id_externo = $8 AND conta_id = $9`, [
              order.X, // status
              executedQty,
              avgPrice,
              commission,
...
async function autoMoveOrderOnCompletion(orderId, newStatus, accountId, retryCount = 0) {
  ...
  try {
    ...
    const orderToMove = orderResult.rows[0];
    ...
    if (!orderToMove.id_posicao || orderToMove.id_posicao === 0) {
      ...
      while (retryAttempt < maxRetries && !posId) {
        ...
        if (intermediatePositionCheck.rows[0].id_posicao && intermediatePositionCheck.rows[0].id_posicao !== 0) {
          posId = intermediatePositionCheck.rows[0].id_posicao;
        ...
        if (availablePositions.rows.length > 0) {
          const targetPosition = availablePositions.rows[0];
          ...
          while (positionUpdateTries < 100) {
            try {
              await connection.query(`UPDATE ordens SET id_posicao = $1 WHERE id_externo = $2 AND conta_id = $3`, [targetPosition.id, orderId, accountId]
              );
    ...
    while (deleteTries < 1000) {
      try {
        [deleteResult] = await connection.query(`DELETE FROM ordens WHERE id_externo = $1 AND conta_id = $2`, [orderId, accountId]
        );
        break;
      } catch (error) {
    ...
    if (verifyResult.rows[0].count > 0) {
      console.error(`[ORDER_AUTO_MOVE] ❌ ERRO CRÍTICO: Ainda existem ${verifyResult.rows[0].count} cópia(s) da ordem ${orderId} na tabela ativa após deleção!`);
      return false;
    }
...
async function insertExternalOrder(dbConnection, orderData, accountId) {
  try {
    ...
    if (orderData.R === true || orderData.cp === true) {
      const result = await connection.query(`SELECT id FROM posicoes WHERE simbolo = $1 AND status = $2 AND conta_id = $3`, [orderData.s, 'OPEN', accountId]
      );
      
      if (existingPositions.rows.length > 0) {
        positionId = existingPositions.rows[0].id;
    ...
    const orderInsertData = {
      ...
      status: orderData.X,
      data_hora_criacao: formatDateForPostgreSQL(new Date(orderData.T || Date.now())),
      id_externo: orderData.i.toString(),
      side: orderData.S,
      simbolo: orderData.s,
      tipo_ordem_bot: orderBotType,
      target: target, // <-- Aqui entra o target correto
      reduce_only: orderData.R === true,
      close_position: orderData.cp === true,
      ...
      is_maker_side: orderData.m === true,
      ...
      price_protection: orderData.pP === true,
      ...
      try {
        await connection.query(`
          ...
          SET status = $1, 
              ...
              trade_id = $6,
              realized_profit = $7,
              dados_originais_ws = $8,
              last_update = CURRENT_TIMESTAMP
          WHERE id_externo = $9 AND conta_id = $10
        `, [
...
async function checkPositionClosureAfterOrderExecution(orderId, accountId) {
  try {
    ...
    const order = executedOrder.rows[0];
...
async function searchAndLinkPosition(orderDbId, orderData, accountId) {
  ...
  const searchInterval = setInterval(async () => {
    ...
    try {
      ...
      // ✅ VERIFICAÇÃO INTERMEDIÁRIA 3: Ordem já tem id_posicao$1
      if (orderCheck.rows[0].id_posicao) {
        console.log(`[POSITION_LINK] ✅ Ordem ${orderDbId} já está vinculada à posição ${orderCheck.rows[0].id_posicao}`);
        clearInterval(searchInterval);
        return;
      }
      ...
      if (positions.rows.length > 0) {
        ...
        while (updateTries < 100) {
          try {
            // ✅ VERIFICAÇÃO FINAL ANTES DO UPDATE: Ordem ainda não foi movida$1
            const result = await connection.query(`SELECT id_posicao FROM ordens WHERE id = $1 AND conta_id = $2`, [orderDbId, accountId]
            );
            ...
            if (finalCheck.rows[0].id_posicao) {
              console.log(`[POSITION_LINK] ✅ Ordem ${orderDbId} já foi vinculada à posição ${finalCheck.rows[0].id_posicao} por outro processo`);
              clearInterval(searchInterval);
              return;
            }
            
            // ✅ EXECUTAR UPDATE
            await connection.query(`UPDATE ordens SET id_posicao = $1 WHERE id = $2 AND conta_id = $3`, [position.id, orderDbId, accountId]
            );
